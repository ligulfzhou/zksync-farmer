"""
主要拷贝至
https://github.com/zksync-sdk/zksync2-python/blob/master/examples/02_transfer.py
"""
...
# Set a provider
PROVIDER = "https://zksync2-mainnet.zksync.io"

# Connect to zkSync network
zk_web3 = ZkSyncBuilder.build(PROVIDER)


def get_eth_balance(address: HexAddress) -> float:
    """
...
def transfer_eth(account: LocalAccount, address: HexAddress, amount: float) -> bytes:
    """
    ...
    global_logger.info(f"Fee for transaction is: {estimate_gas * gas_price}")
    ...
    global_logger.info(f"Transaction hash is : {tx_hash.hex()}")

    # Wait for transaction to be included in a block
    tx_receipt = zk_web3.zksync.wait_for_transaction_receipt(
        tx_hash, timeout=240, poll_latency=0.5
    )
    global_logger.info(f"Tx status: {tx_receipt['status']}")
...
#     global_logger.info(f"Balance before transfer : {get_eth_balance(zk_web3, account.address)} ETH")
#
...
#     global_logger.info(f"Balance after transfer : {get_eth_balance(zk_web3, account.address)} ETH")
